...
def display_publisher(publisher_id):
    publisher = Publication.query.filter_by(id=publisher_id).all()
    publisher_books = Book.query.filter_by(pub_id=publisher[0].id).all()

    return render_template('publisher.html', publisher=publisher[0], publisher_books=publisher_books)
